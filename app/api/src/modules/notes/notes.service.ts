...
export class NotesService {
  ...
  async getNotes(reportId: string, user: JwtPayload): Promise<NotePublic[]> {
    ...
    if (!assigned) {
      throw new ForbiddenException({
        code: "FORBIDDEN",
        message: "Bạn không được gán vào báo cáo này",
      });
    }
  ...
  private async createNoteEmployee(
    ...
    if (dto.parentId) {
      ...
      if (!parent) {
        throw new NotFoundException({
          code: "NOT_FOUND",
          message: "Note cha không tồn tại",
        });
      }
      ...
      if (parent.thread_owner_id !== userId) {
        throw new ForbiddenException({
          code: "FORBIDDEN",
          message: "Bạn chỉ có thể reply trong thread của chính mình",
        });
      }
  ...
  async updateNote(
    ...
    if (note.author_id !== user.sub) {
      throw new ForbiddenException({
        code: "FORBIDDEN",
        message: "Bạn không thể sửa note của người khác",
      });
    }
  ...
  async deleteNote(
    ...
    if (user.role !== "super_admin" && note.author_id !== user.sub) {
      throw new ForbiddenException({
        code: "FORBIDDEN",
        message: "Bạn không thể xóa note này",
      });
    }
