    ...
    if (dto.selectedRecipientId) {
      ...
      if (!chosen) {
        throw new BadRequestException({
          code: 'BAD_REQUEST',
          message: 'selectedRecipientId không tìm thấy trong danh sách nhân viên active',
        });
      }
      ...
      try {
        draft = await this.aiService.composeEmailDraft(dto.prompt, activeSummaries);
      } catch (err) {
        throw new HttpException(
          { code: 'SERVICE_UNAVAILABLE', message: 'AI service không khả dụng — không thể soạn nội dung email' },
          HttpStatus.SERVICE_UNAVAILABLE,
        );
      }

      return {
        recipient: { userId: chosen.id, email: chosen.email, name: chosen.name },
